...
describe('Casos de prueba de APIs', () => {
    ...
    it('Consultar detalles de facturación guardados con token válido API | Marlon Jaramillo', () => {
        cy.getBillingDetailsAPI(user.userId, user.token, 200)
    })

    it('Error al intentar consultar detalles de facturación sin token | Marlon Jaramillo', () => {
        cy.getBillingDetailsAPI(user.userId, '', 200)
    })
})
